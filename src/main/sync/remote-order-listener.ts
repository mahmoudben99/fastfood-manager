...
async function resolveCreateOrder(): Promise<((input: CreateRemoteOrderInput) => CreateRemoteOrderResult) | null> {
  ...
  try {
    // WP-F integration (was a @vite-ignore dynamic shim): order-service.ts is now in-tree and
    // electron-free (db is injected), so a static import bundles cleanly under electron-vite.
    // getDb() stays lazy because connection.ts imports electron; the frozen tests inject their
    // own createOrder via the factory and never reach this production resolver.
    const { createOrderService } = await import('../services/order-service')
    const { getDb } = await import('../database/connection')
    const service = createOrderService({ db: getDb() })
    if (typeof service?.createOrder === 'function') {
      resolvedCreateOrder = (input) => service.createOrder(input) as CreateRemoteOrderResult
